...
5. [Passing SSH certificate information to the authentication webhook](#ssh-certificate-information)
6. [X11 forwarding](#port-socket-and-x11-forwarding)
7. [SSH keepalives](#ssh-keepalives)
8. [Health check endpoint](#health-check-endpoint)
9. [Changes to the Prometheus integration](#changes-to-the-prometheus-integration)
10. [Removed the deprecated DockerRun and KubeRun backends](#removal-of-the-deprecated-dockerrun-and-kuberun-backends)
...
From this release support for forward and reverse port forwarding is supported natively. For these features the ContainerSSH agent has to be enabled as the agent acts as the entry & exit points of the connections inside the container.
